...
{capture assign='headContent'}
    {if $pageNo < $pages}
        <link rel="next" href="{link controller='WickedAvatar'}pageNo={@$pageNo+1}&{@$canonicalURLParameters}{/link}">
    {/if}
    {if $pageNo > 1}
        <link rel="prev" href="{link controller='WickedAvatar'}{if $pageNo > 2}pageNo={@$pageNo-1}&{/if}{@$canonicalURLParameters}{/link}">
    {/if}
    <link rel="canonical" href="{link controller='WickedAvatar'}{if $pageNo > 1}pageNo={@$pageNo}&{/if}{@$canonicalURLParameters}{/link}">
{/capture}

{capture assign='sidebarRight'}
    <section class="jsOnly box">
        <form method="post" action="{link controller='WickedAvatar'}{/link}">
...
{hascontent}
    <div class="paginationTop">
        {content}
            ...
            {pages print=true assign=pagesLinks controller='WickedAvatar' link="pageNo=%d&sortField=$sortField&sortOrder=$sortOrder$linkParameters"}
        {/content}
    </div>
{/hascontent}
...
{if $objects|count}
    <div class="section tabularBox">
        <table class="table">
            <thead>
                <tr>
                    <th class="columnUser{if $sortField == 'username'} active {@$sortOrder}{/if}" colspan="2"><a href="{link controller='WickedAvatar'}pageNo={@$pageNo}&sortField=username&sortOrder={if $sortField == 'username' && $sortOrder == 'ASC'}DESC{else}ASC{/if}{@$linkParameters}{/link}">{lang}wcf.user.username{/lang}</a></th>
                    <th class="columnLastActivity columnText{if $sortField == 'lastActivityTime'} active {@$sortOrder}{/if}"><a href="{link controller='WickedAvatar'}pageNo={@$pageNo}&sortField=lastActivityTime&sortOrder={if $sortField == 'lastActivityTime' && $sortOrder == 'ASC'}DESC{else}ASC{/if}{@$linkParameters}{/link}">{lang}wcf.wicked.lastActivityTime{/lang}</a></th>
                    <th class="columnAvatarExpires columnText{if $sortField == 'disableAvatarExpires'} active {@$sortOrder}{/if}"><a href="{link controller='WickedAvatar'}pageNo={@$pageNo}&sortField=disableAvatarExpires&sortOrder={if $sortField == 'disableAvatarExpires' && $sortOrder == 'ASC'}DESC{else}ASC{/if}{@$linkParameters}{/link}">{lang}wcf.wicked.expires{/lang}</a></th>
                    <th class="columnAvatarReason columnText{if $sortField == 'disableAvatarReason'} active {@$sortOrder}{/if}"><a href="{link controller='WickedAvatar'}pageNo={@$pageNo}&sortField=disableAvatarReason&sortOrder={if $sortField == 'disableAvatarReason' && $sortOrder == 'ASC'}DESC{else}ASC{/if}{@$linkParameters}{/link}">{lang}wcf.wicked.reason{/lang}</a></th>
            ...
            <tbody>
                {foreach from=$objects item=userProfile}
                    <tr class="statementTableRow">
                        ...
                        <td class="columnAvatarExpires columnText">{if $userProfile->disableAvatarExpires}{@$userProfile->banExpires|date}{else}{lang}wcf.wicked.expires.no{/lang}{/if}</td>
                        <td class="columnAvatarReason columnText">{if $userProfile->disableAvatarReason}{$userProfile->disableAvatarReason}{else}{lang}wcf.wicked.reason.no{/lang}{/if}</td>

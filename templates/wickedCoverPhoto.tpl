...
{capture assign='headContent'}
	{if $pageNo < $pages}
		<link rel="next" href="{link controller='WickedCoverPhoto'}pageNo={@$pageNo+1}&{@$canonicalURLParameters}{/link}">
	{/if}
	{if $pageNo > 1}
		<link rel="prev" href="{link controller='WickedCoverPhoto'}{if $pageNo > 2}pageNo={@$pageNo-1}&{/if}{@$canonicalURLParameters}{/link}">
	{/if}
	<link rel="canonical" href="{link controller='WickedCoverPhoto'}{if $pageNo > 1}pageNo={@$pageNo}&{/if}{@$canonicalURLParameters}{/link}">
{/capture}

{capture assign='sidebarRight'}
	<section class="jsOnly box">
		<form method="post" action="{link controller='WickedCoverPhoto'}{/link}">
...
{hascontent}
	<div class="paginationTop">
		{content}
			...
			{pages print=true assign=pagesLinks controller='WickedCoverPhoto' link="pageNo=%d&sortField=$sortField&sortOrder=$sortOrder$linkParameters"}
		{/content}
	</div>
{/hascontent}
...
{if $objects|count}
	<div class="section tabularBox">
		<table class="table">
			<thead>
				<tr>
					<th class="columnUser{if $sortField == 'username'} active {@$sortOrder}{/if}" colspan="2"><a href="{link controller='WickedCoverPhoto'}pageNo={@$pageNo}&sortField=username&sortOrder={if $sortField == 'username' && $sortOrder == 'ASC'}DESC{else}ASC{/if}{@$linkParameters}{/link}">{lang}wcf.user.username{/lang}</a></th>
					<th class="columnLastActivity columnText{if $sortField == 'lastActivityTime'} active {@$sortOrder}{/if}"><a href="{link controller='WickedCoverPhoto'}pageNo={@$pageNo}&sortField=lastActivityTime&sortOrder={if $sortField == 'lastActivityTime' && $sortOrder == 'ASC'}DESC{else}ASC{/if}{@$linkParameters}{/link}">{lang}wcf.wicked.lastActivityTime{/lang}</a></th>
					<th class="columnCoverPhotoExpires columnText{if $sortField == 'disableCoverPhotoExpires'} active {@$sortOrder}{/if}"><a href="{link controller='WickedCoverPhoto'}pageNo={@$pageNo}&sortField=disableCoverPhotoExpires&sortOrder={if $sortField == 'disableCoverPhotoExpires' && $sortOrder == 'ASC'}DESC{else}ASC{/if}{@$linkParameters}{/link}">{lang}wcf.wicked.expires{/lang}</a></th>
					<th class="columnCoverPhotoReason columnText{if $sortField == 'disableCoverPhotoReason'} active {@$sortOrder}{/if}"><a href="{link controller='WickedCoverPhoto'}pageNo={@$pageNo}&sortField=disableCoverPhotoReason&sortOrder={if $sortField == 'disableCoverPhotoReason' && $sortOrder == 'ASC'}DESC{else}ASC{/if}{@$linkParameters}{/link}">{lang}wcf.wicked.reason{/lang}</a></th>
			...
			<tbody>
				{foreach from=$objects item=userProfile}
					<tr class="statementTableRow">
						...
						<td class="columnCoverPhotoExpires columnText">{if $userProfile->disableCoverPhotoExpires}{@$userProfile->banExpires|date}{else}{lang}wcf.wicked.expires.no{/lang}{/if}</td>
						<td class="columnCoverPhotoReason columnText">{if $userProfile->disableCoverPhotoReason}{$userProfile->disableCoverPhotoReason}{else}{lang}wcf.wicked.reason.no{/lang}{/if}</td>

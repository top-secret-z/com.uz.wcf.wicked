...
{capture assign='headContent'}
    {if $pageNo < $pages}
        <link rel="next" href="{link controller='WickedSuspension'}pageNo={@$pageNo+1}&{@$canonicalURLParameters}{/link}">
    {/if}
    {if $pageNo > 1}
        <link rel="prev" href="{link controller='WickedSuspension'}{if $pageNo > 2}pageNo={@$pageNo-1}&{/if}{@$canonicalURLParameters}{/link}">
    {/if}
    <link rel="canonical" href="{link controller='WickedSuspension'}{if $pageNo > 1}pageNo={@$pageNo}&{/if}{@$canonicalURLParameters}{/link}">
{/capture}

{capture assign='sidebarRight'}
    <section class="jsOnly box">
        <form method="post" action="{link controller='WickedSuspension'}{/link}">
...
{hascontent}
    <div class="paginationTop">
        {content}
            ...
            {pages print=true assign=pagesLinks controller='WickedSuspension' link="pageNo=%d&sortField=$sortField&sortOrder=$sortOrder$linkParameters"}
        {/content}
    </div>
{/hascontent}
...
{if $objects|count}
    <div class="section tabularBox">
        <table class="table">
            <thead>
                <tr>
                    <th class="columnUser{if $sortField == 'username'} active {@$sortOrder}{/if}" colspan="2"><a href="{link controller='WickedSuspension'}pageNo={@$pageNo}&sortField=username&sortOrder={if $sortField == 'username' && $sortOrder == 'ASC'}DESC{else}ASC{/if}{@$linkParameters}{/link}">{lang}wcf.user.username{/lang}</a></th>
                    <th class="columnLastActivity columnText{if $sortField == 'lastActivityTime'} active {@$sortOrder}{/if}"><a href="{link controller='WickedSuspension'}pageNo={@$pageNo}&sortField=lastActivityTime&sortOrder={if $sortField == 'lastActivityTime' && $sortOrder == 'ASC'}DESC{else}ASC{/if}{@$linkParameters}{/link}">{lang}wcf.wicked.lastActivityTime{/lang}</a></th>
                    <th class="columnTime columnText{if $sortField == 'time'} active {@$sortOrder}{/if}"><a href="{link controller='WickedSuspension'}pageNo={@$pageNo}&sortField=time&sortOrder={if $sortField == 'time' && $sortOrder == 'ASC'}DESC{else}ASC{/if}{@$linkParameters}{/link}">{lang}wcf.wicked.time{/lang}</a></th>
                    <th class="columnExpires columnText{if $sortField == 'expires'} active {@$sortOrder}{/if}"><a href="{link controller='WickedSuspension'}pageNo={@$pageNo}&sortField=expires&sortOrder={if $sortField == 'expires' && $sortOrder == 'ASC'}DESC{else}ASC{/if}{@$linkParameters}{/link}">{lang}wcf.wicked.expires{/lang}</a></th>
                    <th class="columnTitle columnText{if $sortField == 'title'} active {@$sortOrder}{/if}"><a href="{link controller='WickedSuspension'}pageNo={@$pageNo}&sortField=title&sortOrder={if $sortField == 'title' && $sortOrder == 'ASC'}DESC{else}ASC{/if}{@$linkParameters}{/link}">{lang}wcf.wicked.sanction{/lang}</a></th>
            ...
            <tbody>
                {foreach from=$objects item=suspension}
                    {assign var='userProfile' value=$userProfiles[$suspension->userID]}
                    ...
                    <tr class="statementTableRow">
                        ...
                        <td class="columnTime columnText">{if $suspension->time}{$suspension->time|date}{else}{lang}wcf.wicked.time.no{/lang}{/if}</td>
                        <td class="columnExpires columnText">{if $suspension->expires}{$suspension->expires|date}{else}{lang}wcf.wicked.expires.no{/lang}{/if}</td>
                        <td class="columnTitle columnText">{lang}{$suspension->title}{/lang}</td>

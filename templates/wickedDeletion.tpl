...
{capture assign='headContent'}
	{if $pageNo < $pages}
		<link rel="next" href="{link controller='WickedDeletion'}pageNo={@$pageNo+1}&{@$canonicalURLParameters}{/link}">
	{/if}
	{if $pageNo > 1}
		<link rel="prev" href="{link controller='WickedDeletion'}{if $pageNo > 2}pageNo={@$pageNo-1}&{/if}{@$canonicalURLParameters}{/link}">
	{/if}
	<link rel="canonical" href="{link controller='WickedDeletion'}{if $pageNo > 1}pageNo={@$pageNo}&{/if}{@$canonicalURLParameters}{/link}">
{/capture}

{capture assign='sidebarRight'}
	<section class="jsOnly box">
		<form method="post" action="{link controller='WickedDeletion'}{/link}">
...
{hascontent}
	<div class="paginationTop">
		{content}
			...
			{pages print=true assign=pagesLinks controller='WickedDeletion' link="pageNo=%d&sortField=$sortField&sortOrder=$sortOrder$linkParameters"}
		{/content}
	</div>
{/hascontent}
...
{if $objects|count}
	<div class="section tabularBox">
		<table class="table">
			<thead>
				<tr>
					<th class="columnUser{if $sortField == 'username'} active {@$sortOrder}{/if}" colspan="2"><a href="{link controller='WickedDeletion'}pageNo={@$pageNo}&sortField=username&sortOrder={if $sortField == 'username' && $sortOrder == 'ASC'}DESC{else}ASC{/if}{@$linkParameters}{/link}">{lang}wcf.user.username{/lang}</a></th>
					<th class="columnLastActivity columnText{if $sortField == 'lastActivityTime'} active {@$sortOrder}{/if}"><a href="{link controller='WickedDeletion'}pageNo={@$pageNo}&sortField=lastActivityTime&sortOrder={if $sortField == 'lastActivityTime' && $sortOrder == 'ASC'}DESC{else}ASC{/if}{@$linkParameters}{/link}">{lang}wcf.wicked.lastActivityTime{/lang}</a></th>
					<th class="columnQuitStarted columnText{if $sortField == 'quitStarted'} active {@$sortOrder}{/if}"><a href="{link controller='WickedDeletion'}pageNo={@$pageNo}&sortField=quitStarted&sortOrder={if $sortField == 'quitStarted' && $sortOrder == 'ASC'}DESC{else}ASC{/if}{@$linkParameters}{/link}">{lang}wcf.wicked.quitStarted{/lang}</a></th>
				</tr>
			</thead>
			...
			<tbody>
				{foreach from=$objects item=userProfile}
					<tr class="statementTableRow">
						...
						<td class="columnQuitStarted columnText">{@$userProfile->quitStarted + 7 * 86400|date}</td>
					</tr>
				{/foreach}
			</tbody>
		</table>
